/****************************************************************************************
SPBUN (SPBU Nelayan) - Data generation FULL_SAMPLE
...
* Outputs
global outdir "$ROOT/Output/iv2sls"
cap mkdir "$outdir"

cap log close
log using "$outdir/run_spbun_iv2sls.log", replace text
...
preserve
    ...
    * auto-detect lat/lon columns (handles Lintang/Bujur OR Latitude/Longitude)
    ...
    local loncol ""
    foreach v of varlist _all {
        if "`latcol'"=="" & (strpos(lower("`v'"),"lintang") | strpos(lower("`v'"),"lat")) local latcol `v'
        if "`loncol'"=="" & (strpos(lower("`v'"),"bujur")   | strpos(lower("`v'"),"lon") | strpos(lower("`v'"),"long")) local loncol `v'
    }
    if "`latcol'"=="" | "`loncol'"=="" {
...
preserve
    ...
    local loncol ""
    foreach v of varlist _all {
        if "`latcol'"=="" & strpos(lower("`v'"),"lat") local latcol `v'
        if "`loncol'"=="" & (strpos(lower("`v'"),"lon") | strpos(lower("`v'"),"long")) local loncol `v'
    }
    if "`latcol'"=="" | "`loncol'"=="" {
