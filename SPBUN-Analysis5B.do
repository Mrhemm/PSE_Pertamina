/****************************************************************************************
SPBUN (SPBU Nelayan) exposure → welfare outcomes (Using Ker10_w1)
...
if ($DO_M0==1 | $DO_M1==1 | $DO_M2==1 | $DO_M3==1) {
    log using "$outdir/logs/Ker10_V1_run_iv_suite.log", replace text
}
else {
	log using "$outdir/logs/Ker10_V2_run_iv_suite.log", replace text
}
...
gen str18  outcome = ""
gen str12  spec    = ""
gen double Ker10_used= .
gen double W_hat   = .
gen double uhat    = .
gen double beta_vill = .
gen double A_contrib = .

save "$VMASTER1X2", replace

tempfile VMASTER2X2
global VMASTER2X2 "`VMASTER2X2'"
...
program define _run_one_sample
    ...
    *----------------------------
    cap confirm variable Ker10_w1
    if _rc {
        cap confirm variable D_spbun
        if _rc {
            di as err "Missing Ker10_w1 and D_spbun; cannot construct exposure."
            exit 198
        }
        gen double Ker10_w1 = -ln(1 + D_spbun) if !missing(D_spbun)
    }
    ...
    * A) Clean categorical
    ...
    tempvar __es
    gen byte `__es' = !missing(Ker10_w1, Z_tpi, Z_tbbm, Z_pel, kab)
    ...
    ****************************************************************************************/
    if $DO_M0 == 1 {
        noi di as txt "-> Ker10 V1 M0 First stage (`tag')"
        qui reghdfe Ker10_w1 Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
        est store M0_`tag'
        esttab M0_`tag' using "$outdir/tables/Ker10_V1_M0_firststage_`tag'.rtf", replace ///
            se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
            title("Ker10 M0 First stage v1: Ker10_w1 on Z + X + FE (`tag')") ///
            scalars(N r2) compress
    }
	
	if $DO_M0 == 0 {
        noi di as txt "-> Ker10 V2 M0 First stage (`tag')"
        qui reghdfe Ker10_w1 Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
        est store M0_`tag'
        esttab M0_`tag' using "$outdir/tables/Ker10_V2_M0_firststage_`tag'.rtf", replace ///
            se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
            title("Ker10 M0 First stage v2: Ker10_w1 on Z + X + FE (`tag')") ///
            scalars(N r2) compress
    }
    ...
    if $DO_M1 == 1 {
        ...
        foreach Y of local YLIST_S {
            ...
            foreach s of local M1SPECS {
                ...
                noi di as txt "-> Ker10 M1 Homog IV v1: `Y' (`tag') [ `scode' ]"

                cap noisily ivreghdfe `Y' (Ker10_w1 = Z_pel Z_tbbm Z_tpi) `X_this', `IVOPTS' `PART_this'
				...
				local estname = "Ker10_v1_m1`sshort'`tshort'`ycode'"
				est store `estname'

				esttab `estname' using "$outdir/tables/V1_M1_iv_homog_`tag'_`Y'_`scode'.rtf", replace ///
					se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
					title("Ker10 M1 Homog IV (`scode') v1: `Y' on Ker10_w1 (`tag')") ///
					scalars(N r2) compress

				cap drop W_hat uhat beta_vill A_contrib
				predict double W_hat if e(sample), xb
				predict double uhat  if e(sample), resid
				gen double beta_vill = _b[Ker10_w1] if e(sample)
				gen double A_contrib = beta_vill * Ker10_w1 if e(sample) & !missing(Ker10_w1)

				preserve
					keep $ID Ker10_w1 W_hat uhat beta_vill A_contrib
					...
					rename Ker10_w1 Ker10_used
					append using "$VMASTER1X2"
					save "$VMASTER1X2", replace
				restore
                ...
                * Run heterogeneity only for FULL controls (IVX4)
                if "`s'" == "full" & "`H_S'" != "" {
                    ...
                    noi di as txt "-> Ker10 M1 Hetero IV v1: `Y' (`tag') [ IVX4 ] with H = `H_S'"

                    local AENDO "Ker10_w1"
                    ...
                    foreach h of local H_S {
                        ...
                        cap drop A_X_`h'
                        gen double A_X_`h' = Ker10_w1 * `h' if !missing(Ker10_w1, `h')
                    ...
                    * FULL controls for heterogeneity
					...
					local estname = "Ker10_v1_m1X4`tshort'`ycode'"
					est store `estname'

					esttab `estname' using "$outdir/tables/Ker10_V1_M1_iv_hetero_`tag'_`Y'_IVX4.rtf", replace ///
						se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
						title("Ker10 M1 Hetero IV (IVX4) v1: `Y' on Ker10_w1 + A×H (`tag')") ///
						scalars(N r2) compress
					...
					preserve
						cap drop Ker10_used
						gen double Ker10_used = Ker10_w1 if e(sample) & !missing(Ker10_w1)
						keep $ID Ker10_used W_hat uhat beta_vill A_contrib `H_S'
						cap confirm string variable $ID
						if _rc {
							tostring $ID, replace usedisplayformat format(%18.0g)
						}
	...
	if $DO_M1 == 0 {
        ...
        foreach Y of local YLIST_S {
            ...
            foreach s of local M1SPECS {
                ...
                noi di as txt "-> Ker10 M1 Homog IV v2: `Y' (`tag') [ `scode' ]"
				...
				local estname = "Ker10_v2_m1`sshort'`tshort'`ycode'"
				est store `estname'

				esttab `estname' using "$outdir/tables/Ker10_V2_M1_iv_homog_`tag'_`Y'_`scode'.rtf", replace ///
					se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
					title("Ker10 M1 Homog IV (`scode') v2: `Y' on Ker10_w1 (`tag')") ///
					scalars(N r2) compress
                ...
                * Run heterogeneity only for FULL controls (IVX4)
                if "`s'" == "full" & "`H_S'" != "" {
                    ...
                    noi di as txt "-> Ker10 M1 Hetero IV v2: `Y' (`tag') [ IVX4 ] with H = `H_S'"
                    ...
                    * FULL controls for heterogeneity
					...
					local estname = "Ker10_v2_m1X4`tshort'`ycode'"
					est store `estname'

					esttab `estname' using "$outdir/tables/Ker10_V2_M1_iv_hetero_`tag'_`Y'_IVX4.rtf", replace ///
						se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
						title("Ker10 M1 Hetero IV (IVX4) v2: `Y' on Ker10_w1 + A×H (`tag')") ///
						scalars(N r2) compress
    ...
    ****************************************************************************************/
    if $DO_M2 == 1 {
        foreach Y of local YLIST_S {
            noi di as txt "-> Ker10 M2 Reduced form v1: `Y' (`tag')"
            cap noisily reghdfe `Y' Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M2_`tag'_`Y'
                esttab M2_`tag'_`Y' using "$outdir/tables/Ker10_V1_M2_reducedform_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("Ker10 M2 Reduced form: `Y' on Z + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
	...
	if $DO_M2 == 0 {
        foreach Y of local YLIST_S {
            noi di as txt "-> Ker10 M2 Reduced form v2: `Y' (`tag')"
            cap noisily reghdfe `Y' Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M2_`tag'_`Y'
                esttab M2_`tag'_`Y' using "$outdir/tables/Ker10_V2_M2_reducedform_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("Ker10 M2 Reduced form v2: `Y' on Z + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
    ...
    ****************************************************************************************/
    if $DO_M3 == 1 {
        foreach Y of local YLIST_S {
            noi di as txt "-> Ker10 M3 OLS v1: `Y' (`tag')"
            cap noisily reghdfe `Y' Ker10_w1 `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M3_`tag'_`Y'
                esttab M3_`tag'_`Y' using "$outdir/tables/Ker10_V1_M3_ols_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("Ker10 M3 OLS v1: `Y' on Ker10_w1 + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
	...
	if $DO_M3 == 0 {
        foreach Y of local YLIST_S {
            noi di as txt "-> Ker10 M3 OLS v2: `Y' (`tag')"
            cap noisily reghdfe `Y' Ker10_w1 `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M3_`tag'_`Y'
                esttab M3_`tag'_`Y' using "$outdir/tables/Ker10_V2_M3_ols_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("Ker10 M3 OLS v2: `Y' on Ker10_w1 + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
...
foreach v in Ker10_used W_hat uhat beta_vill A_contrib {
    cap confirm variable `v'
    if _rc gen double `v' = .
}

drop if missing($ID)
order $ID sample outcome spec Ker10_used W_hat uhat beta_vill A_contrib
compress

save "$outdir/village_values/Ker10_village_values_long_v1.dta", replace
export delimited using "$outdir/village_values/Ker10_village_values_long_v1.csv", replace

use "$VMASTER2X2", clear
...
save "$outdir/village_values/Ker10_village_values_long_v2.dta", replace
export delimited using "$outdir/village_values/Ker10_village_values_long_v2.csv", replace
