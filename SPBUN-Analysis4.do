/****************************************************************************************
SPBUN (SPBU Nelayan) exposure → welfare outcomes 
...
if ($DO_M0==1 | $DO_M1==1 | $DO_M2==1 | $DO_M3==1) {
    log using "$outdir/logs/V1_run_iv_suite.log", replace text
}
else {
	log using "$outdir/logs/V2_run_iv_suite.log", replace text
}
...
save "$VMASTER1", replace

tempfile VMASTER2
global VMASTER2 "`VMASTER2'"
...
program define _run_one_sample
    ...
    *----------------------------
    cap confirm variable A_v_log
    if _rc {
        cap confirm variable D_spbun
        if _rc {
            di as err "Missing A_v_log and D_spbun; cannot construct exposure."
            exit 198
        }
    ...
    * A) Clean categorical
    ...
    tempvar __es
    gen byte `__es' = !missing(A_v_log, Z_tpi, Z_tbbm, Z_pel, kab)
    ...
    ****************************************************************************************/
    if $DO_M0 == 1 {
        noi di as txt "-> V1 M0 First stage (`tag')"
        qui reghdfe A_v_log Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
        est store M0_`tag'
        esttab M0_`tag' using "$outdir/tables/V1_M0_firststage_`tag'.rtf", replace ///
            se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
            title("M0 First stage v1: A_v_log on Z + X + FE (`tag')") ///
            scalars(N r2) compress
    }
	
	if $DO_M0 == 0 {
        noi di as txt "-> V2 M0 First stage (`tag')"
        qui reghdfe A_v_log Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
        est store M0_`tag'
        esttab M0_`tag' using "$outdir/tables/V2_M0_firststage_`tag'.rtf", replace ///
            se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
            title("M0 First stage v2: A_v_log on Z + X + FE (`tag')") ///
            scalars(N r2) compress
    }
    ...
    if $DO_M1 == 1 {
        ...
        foreach Y of local YLIST_S {
            ...
            foreach s of local M1SPECS {
                ...
                noi di as txt "-> M1 Homog IV v1: `Y' (`tag') [ `scode' ]"

                cap noisily ivreghdfe `Y' (A_v_log = Z_pel Z_tbbm Z_tpi) `X_this', `IVOPTS' `PART_this'
				...
				local estname = "v1_m1`sshort'`tshort'`ycode'"
				est store `estname'

				esttab `estname' using "$outdir/tables/V1_M1_iv_homog_`tag'_`Y'_`scode'.rtf", replace ///
					se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
					title("M1 Homog IV (`scode') v1: `Y' on A_v_log (`tag')") ///
					scalars(N r2) compress

				cap drop W_hat uhat beta_vill A_contrib
				predict double W_hat if e(sample), xb
				predict double uhat  if e(sample), resid
				gen double beta_vill = _b[A_v_log] if e(sample)
				gen double A_contrib = beta_vill * A_v_log if e(sample) & !missing(A_v_log)

				preserve
					keep $ID A_v_log W_hat uhat beta_vill A_contrib
					...
					rename A_v_log A_used
					append using "$VMASTER1"
					save "$VMASTER1", replace
				restore
                ...
                * Run heterogeneity only for FULL controls (IVX4)
                if "`s'" == "full" & "`H_S'" != "" {
                    ...
                    noi di as txt "-> M1 Hetero IV v1: `Y' (`tag') [ IVX4 ] with H = `H_S'"
                    ...
                    foreach h of local H_S {
                        ...
                        cap drop A_X_`h'
                        gen double A_X_`h' = A_v_log * `h' if !missing(A_v_log, `h')
                    ...
                    * FULL controls for heterogeneity
					...
					local estname = "v1_m1X4`tshort'`ycode'"
					est store `estname'

					esttab `estname' using "$outdir/tables/V1_M1_iv_hetero_`tag'_`Y'_IVX4.rtf", replace ///
						se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
						title("M1 Hetero IV (IVX4) v1: `Y' on A_v_log + A×H (`tag')") ///
						scalars(N r2) compress
					...
					preserve
						cap drop A_used
						gen double A_used = A_v_log if e(sample) & !missing(A_v_log)
						keep $ID A_used W_hat uhat beta_vill A_contrib `H_S'
						cap confirm string variable $ID
						if _rc {
							tostring $ID, replace usedisplayformat format(%18.0g)
						}
	...
	if $DO_M1 == 0 {
        ...
        foreach Y of local YLIST_S {
            ...
            foreach s of local M1SPECS {
                ...
                noi di as txt "-> M1 Homog IV v2: `Y' (`tag') [ `scode' ]"
				...
				local estname = "v2_m1`sshort'`tshort'`ycode'"
				est store `estname'

				esttab `estname' using "$outdir/tables/V2_M1_iv_homog_`tag'_`Y'_`scode'.rtf", replace ///
					se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
					title("M1 Homog IV (`scode') v2: `Y' on A_v_log (`tag')") ///
					scalars(N r2) compress
                ...
                * Run heterogeneity only for FULL controls (IVX4)
                if "`s'" == "full" & "`H_S'" != "" {
                    ...
                    noi di as txt "-> M1 Hetero IV v2: `Y' (`tag') [ IVX4 ] with H = `H_S'"
                    ...
                    * FULL controls for heterogeneity
					...
					local estname = "v2_m1X4`tshort'`ycode'"
					est store `estname'

					esttab `estname' using "$outdir/tables/V2_M1_iv_hetero_`tag'_`Y'_IVX4.rtf", replace ///
						se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
						title("M1 Hetero IV (IVX4) v2: `Y' on A_v_log + A×H (`tag')") ///
						scalars(N r2) compress
    ...
    ****************************************************************************************/
    if $DO_M2 == 1 {
        foreach Y of local YLIST_S {
            noi di as txt "-> M2 Reduced form v1: `Y' (`tag')"
            cap noisily reghdfe `Y' Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M2_`tag'_`Y'
                esttab M2_`tag'_`Y' using "$outdir/tables/V1_M2_reducedform_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("M2 Reduced form: `Y' on Z + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
	...
	if $DO_M2 == 0 {
        foreach Y of local YLIST_S {
            noi di as txt "-> M2 Reduced form v2: `Y' (`tag')"
            cap noisily reghdfe `Y' Z_pel Z_tbbm Z_tpi `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M2_`tag'_`Y'
                esttab M2_`tag'_`Y' using "$outdir/tables/V2_M2_reducedform_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("M2 Reduced form v2: `Y' on Z + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
    ...
    ****************************************************************************************/
    if $DO_M3 == 1 {
        foreach Y of local YLIST_S {
            noi di as txt "-> M3 OLS v1: `Y' (`tag')"
            cap noisily reghdfe `Y' A_v_log `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M3_`tag'_`Y'
                esttab M3_`tag'_`Y' using "$outdir/tables/V1_M3_ols_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("M3 OLS v1: `Y' on A_v_log + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
	...
	if $DO_M3 == 0 {
        foreach Y of local YLIST_S {
            noi di as txt "-> M3 OLS v2: `Y' (`tag')"
            cap noisily reghdfe `Y' A_v_log `X_S', absorb($FE) vce(cluster $CLUST)
            if !_rc {
                est store M3_`tag'_`Y'
                esttab M3_`tag'_`Y' using "$outdir/tables/V2_M3_ols_`tag'_`Y'.rtf", replace ///
                    se b(%9.4f) se(%9.4f) star(* 0.10 ** 0.05 *** 0.01) ///
                    title("M3 OLS v2: `Y' on A_v_log + X + FE (`tag')") ///
                    scalars(N r2) compress
            }
        }
    }
...
foreach v in A_used W_hat uhat beta_vill A_contrib {
    cap confirm variable `v'
    if _rc gen double `v' = .
}

drop if missing($ID)
order $ID sample outcome spec A_used W_hat uhat beta_vill A_contrib
compress

save "$outdir/village_values/village_values_long_v1.dta", replace
export delimited using "$outdir/village_values/village_values_long_v1.csv", replace
...
save "$outdir/village_values/village_values_long_v2.dta", replace
export delimited using "$outdir/village_values/village_values_long_v2.csv", replace
